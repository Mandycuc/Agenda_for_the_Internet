...
if __name__ == "__main__":
    # 加载Word2Vec模型
    model = Word2Vec.load("D:\MANDY\毕业论文\模型\模型1\word2vec_model_all.bin")
    
    # 读取关键词列表
    keywords = read_keywords("tf_union.txt")
    
    # 计算相似度矩阵
    similarity_matrix = calculate_similarity_matrix(model, keywords)
    
    # 导出为DL格式
    export_to_dl_format(similarity_matrix, keywords, "union.dl")
